\subsubsection{QEMU Device Model (Qdev)}
...
on top of QOM, because of this, each QDev device is itself a QOM object, meaning it inherits 
...
The Figure \ref{fig:dev_life} show this process.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Resources/Imagenes/qdev_life.png}
    \caption{Devices Lifecycle}
    \label{fig:dev_life}
\end{figure}
...
specific addresses on the bus. Table \ref{tab:qemu_bus_types} shows a list of common QEMU buses 
and their purpose.
...
\begin{table}[H]
    ...
    \begin{tabular}{l r}
        \toprule
        \textbf{Bus Type} & \textbf{Purpose} \\
        \midrule
        \textbf{System Bus (sysbus)} & Default; memory-mapped devices \\
        \textbf{I$^2$C Bus} & I$^2$C protocol; slave devices \\
        \textbf{PCI Bus} & Peripheral Component Interconnect \\
        \textbf{SPI Bus} & Serial Peripheral Interface \\
    \bottomrule
    \end{tabular}
    \caption{QEMU Bus Types and Their Purposes}
    \label{tab:qemu_bus_types}
\end{table}
...
bare-metal microcontroller emulation than for system-level scenarios. Similar to the QOM API, in Annex~\ref{ann:qemu_c_structures} there are several key elements of the QDev API that 
will be used and mentioned throughout the thesis.