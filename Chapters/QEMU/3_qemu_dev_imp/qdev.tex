...
atop QOM that abstracts the complexities of emulated hardware devices, buses, and 
their interactions. It's important to denote that, because QDev is implemented as a 
QOM-based extension, each QDev device is itself a QOM object, meaning it inherits 
QOM's type system, introspection, and dynamic property mechanisms.

Qdev works as a layer that unify the description, creation, connection, and 
configuration of the emulated hardware components—ensuring that every device, 
regardless of its specific peripheral function, follows consistent patterns for initialization, 
